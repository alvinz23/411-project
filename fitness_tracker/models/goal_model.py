...
def update_goal(goal_id: int, goal_type: str = None, target_value: float = None, end_date: str = None):
    """
    Updates an existing goal's attributes.
    ...
    Raises:
        ...
        sqlite3.Error: If there is an issue with the database.
    """
    if not any([goal_type, target_value, end_date]):
        raise ValueError("At least one field (goal_type, target_value, end_date) must be provided to update a goal.")
    ...
    # Add the goal ID to the parameters list
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            logger.info("Goal ID %d updated successfully.", goal_id)
    except sqlite3.Error as e:
        logger.error("Error updating goal ID %d: %s", goal_id, str(e))
        raise e
